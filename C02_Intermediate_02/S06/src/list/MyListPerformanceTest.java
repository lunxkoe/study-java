...
public class MyListPerformanceTest {
    ...
    public static void main(String[] args) {
        ...
        addFirst(new MyArrayList<>(), size); // 찾는데 O(1), 데이터 추가(밀기) O(N): O(N)
        addMid(new MyArrayList<>(), size); // 찾는데 O(1), 데이터 추가(밀기) O(N/2): O(N/2)
        ...
        addFirst(new MyLinkedList<>(), size); // 찾는데 O(1), 데이터 추가 O(1): O(1)
        addMid(new MyLinkedList<>(), size); // 찾는데 O(N/2), 데이터 추가 O(1): O(N/2)
