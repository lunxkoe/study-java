...
public class JavaListPerformanceTest {
    ...
    public static void main(String[] args) {
        ...
        addFirst(new ArrayList<>(), size); // 찾는데 O(1), 데이터 추가(밀기) O(N): O(N)
        addMid(new ArrayList<>(), size); // 찾는데 O(1), 데이터 추가(밀기) O(N/2): O(N/2)
        ...
        addFirst(new LinkedList<>(), size); // 찾는데 O(1), 데이터 추가 O(1): O(1)
        addMid(new LinkedList<>(), size); // 찾는데 O(N/2), 데이터 추가 O(1): O(N/2)
